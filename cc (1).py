...
SUX_CONFIG_FILE = os.path.join(BASE_DIR, "sux_config.json")
SESSIONS_DIR = os.path.join(BASE_DIR, "bins_sessions")
...
if os.path.exists(os.path.join(BASE_DIR, "bins.txt")):
    BINS_FILE = os.path.join(BASE_DIR, "bins.txt")
else:
    BINS_FILE = os.path.join(BASE_DIR, "bins_full.txt")
...
def _get_memory_mb():
    """Return (total_mb, available_mb). Linux VPS reads /proc/meminfo."""
    try:
...
def auto_tune_runtime_settings():
    ...
    Works on 4GB or 8GB Hostinger VPS without manual config.
    """
...
FILE_TOO_LARGE_MSG = "❌ File is too large. Maximum allowed is 10 MB."
TEXT_TOO_LARGE_MSG = "❌ Text is too large. Maximum allowed is 10 MB."
